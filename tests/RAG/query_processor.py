...
from knowledge_base.processor.vector_store_processor import VectorStoreProcessor
from collections import defaultdict
import numpy as np
import json

# TODO: sort searching result according to original order
...
class QueryProcessor:
    ...
    def query(self, query_text: str, k: int = 5) -> List[Dict]:
        ...
        for i, group_results in enumerate(metadata_groups.values()):
            ...
            aggregated_result = {
                ...
                'metadata': group_results[0].get('metadata', {}),
    ...
    def _gather_by_metadata(self, similarity_scores: List[float], results: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        for result in results:
            if result['type'] != 'wiki':
                metadata_key = json.dumps(result['metadata'])
            else:
                metadata_key = result['name']
            metakey_set.add(metadata_key)
        
        # Find all chunks with matching metadata in original data
        for chunk in self.vector_store.chunks:
            if chunk['type'] != 'wiki':
                metadata_key = json.dumps(chunk['metadata'])
            else:
                metadata_key = chunk['name']
            if metadata_key in metakey_set:
                metadata_groups[metadata_key].append(chunk)
